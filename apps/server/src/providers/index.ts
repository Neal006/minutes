...
const STTS = ['openrouter', 'whisper', 'mock'] as const;

/**
 * Picks providers from env. Defaults: OpenRouter (free models) when OPENROUTER_API_KEY is set,
 * Claude when only ANTHROPIC_API_KEY is set; Whisper-compatible STT when an STT/OpenAI key is set.
 */
...
export function createAi(env = process.env, hooks: AiHooks = {}): { ai: Ai; description: string } {
  ...
  const sttName = env.STT_PROVIDER ?? (env.STT_API_KEY || env.OPENAI_API_KEY ? 'whisper' : llmName === 'mock' ? 'mock' : 'openrouter');
  ...
  const stt: Stt = sttName === 'openrouter' ? openRouterStt(openRouter, env, hooks.onCall) : sttName === 'whisper' ? whisperStt(env) : mock;
